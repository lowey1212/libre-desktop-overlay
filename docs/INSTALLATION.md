...
Use **Food database** to edit the offline UK list or select USA to search the official [USDA FoodData Central API](https://fdc.nal.usda.gov/api-guide/) with your own API key. The key is stored in Windows Credential Manager. The official UK [CoFID dataset](https://www.gov.uk/government/publications/composition-of-foods-integrated-dataset-cofid) is a downloadable dataset rather than a public search API. **Export data** creates a CSV or JSON timeline for review with your GP, nurse, or official medical software.
