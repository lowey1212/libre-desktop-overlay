...
- Food and insulin events appear as markers on the recent glucose graph and can be reviewed or edited under **Manage events**; double-click an entry or use **Edit selected**.
...
In Juggluco, open **Settings → Exchange data → Web server**, enable the web server, unset **Local only**, and use port **17580**. Keep the phone and PC on the same local network. Enter the phone's local IP or hostname in **Live source → Juggluco — Local**. An API secret is strongly recommended. Never expose or port-forward port 17580 to the public internet.
